...
if [ "`grep -i ubuntu /etc/os-release 2>/dev/null`" != "" ]; then
	...
	if [ "`echo $OS_VERSION|egrep '^(24|22|20)'`" == "" ]; then
...
#        	apt -y install nvidia-driver-390 > /dev/null 2>&1
        	apt -y install nvidia-driver-545 > /dev/null 2>&1
	fi
	...
	if [ -d /etc/apt/sources.list.d ]; then
		if [ ! -f /etc/apt/sources.list.d/endpoint-verification.list ]; then
			echo "deb [arch=amd64 signed-by=/usr/share/keyrings/google-endpoint-verification.gpg] https://packages.cloud.google.com/apt endpoint-verification main" > /etc/apt/sources.list.d/endpoint-verification.list
			curl -s -m 30 https://packages.cloud.google.com/apt/doc/apt-key.gpg | sudo tee /etc/apt/trusted.gpg.d/endpoint-verification.gpg - > /dev/null 2>&1
			apt update > /dev/null 2>&1
			apt -y install endpoint-verification > /dev/null 2>&1
		fi

#		if [ "`grep '^deb.*repo.fortinet.com/repo/forticlient' /etc/apt/sources.list`" = "" ]; then
#			echo 'deb [arch=amd64] https://repo.fortinet.com/repo/forticlient/7.2/ubuntu/ /stable multiverse' >> /etc/apt/sources.list
#			wget -O - https://repo.fortinet.com/repo/forticlient/7.2/ubuntu/DEB-GPG-KEY 2>/dev/null | apt-key add - > /dev/null 2>&1 || true
#			apt -y update > /dev/null 2>&1
#			apt install -y forticlient > /dev/null 2>&1
#		fi
#	fi
	...
	if [ ! -x "/usr/bin/google-chrome-stable" ]; then
		...
		apt-key adv --keyserver keyserver.ubuntu.com --recv-keys C0BA5CE6DC6315A3 > /dev/null 2>&1
